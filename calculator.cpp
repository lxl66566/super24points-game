...
QString calculator::calculate()
{
    ...
    for (auto &op3 : all_operation)
    {
        ...
        // branch cut: all operation are same and the two numbers could be exchange.
        std::ranges::sort(num);
        do
        {
            if (op1.accept_exchange() && num.at(0) > num.at(1)) continue;
            if (op2 == op3 && op3.accept_exchange() && num.at(2) > num.at(3)) continue;
            if (op1 == op2 && op2.accept_exchange() && !(num.at(0)<=num.at(1)&&num.at(1)<=num.at(2))) continue;
            // branch cut:
            try {
...
//                                .arg(op2.get_op_string()).arg(num[2]).arg(op3.get_op_string()).arg(num[3]);
        ...
        do
        {
            if (op3.accept_exchange() && num.at(2) > num.at(3)) continue;
            if (op1.accept_exchange() && num.at(0) > num.at(1)) continue;
            // branch cut
            try{
